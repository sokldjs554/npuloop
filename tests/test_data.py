...
def test_fit_selects_best_checkpoint_on_validation_split(tmp_path):
    ...
    # oneDNN channel blocks (8 on AVX2, 16 on AVX-512). oneDNN 3.12 (torch 2.14.0) writes past the rtus workspace
    # in 1x1 backward_weights when a channels_last, strided conv has fewer input channels than the block: silently
    # wrong diff_weights or a segfault (on the CI runners it surfaced as a stall in loss.backward(), runs 12-16).
    # Report, validated patch and reproducer: docs/upstream/ and tools/onednn_1x1_repro.py.
