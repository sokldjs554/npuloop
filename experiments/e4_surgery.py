...
QAT_LR = float(os.environ.get("NPULOOP_QAT_LR", "0.002"))        # 0.005 diverges on the BN-folded ReLU ResNet (no BN left to stabilise)
...
def part_a(ds, res):
    ...
    for sname in ["per-tensor"]:            # the per-channel PTQ reference comes from E2
        ...
        if not res.has(part="a", model=name, scheme=sname, step="cle+qat"):
            gm = fold_bn(m); equalize(gm)
            qm = prepare(gm, sch); calibrate(qm, calib)
            t = time.time(); lg = qat(qm, ds, epochs=QAT_EPOCHS, lr=QAT_LR, seed=0, steps_per_epoch=QAT_STEPS)
...
def part_b(ds, res):
    ...
    if not res.has(part="b", variant="silu-qat"):
        qm = prepare(m, sch); calibrate(qm, calib); lg = qat(qm, ds, epochs=QAT_EPOCHS, lr=QAT_LR, seed=0, steps_per_epoch=QAT_STEPS)
...
def main():
    ...
    ds = dataset()
    res = Results("e4_surgery", meta=dict(heal_epochs=HEAL_EPOCHS, qat_epochs=QAT_EPOCHS, qat_steps_per_epoch=QAT_STEPS, qat_lr=QAT_LR, int_eval_images=INT_EVAL))
