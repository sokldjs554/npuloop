...
출력 픽셀 코드의 $6.3$--$18.2\%$가 달랐다. 재양자화의 반올림을 제거한
...
이고 시뮬레이터는 그 비용을 예측한 값과 정수 결과의 차이는 최대 약 $6.4\times10^{-4}$~dB다. 이 표본 크기에서는 분해되는 차이지만
($|\Delta|/\mathrm{SE} = 17.6$) 양자화 자체의 PSNR 변화보다 훨씬 작다. 그런데도 출력 픽셀 코드의 $6.3\%$와
$18.2\%$가 $3{,}925$장 전부에서 다르고, 최대 3레벨까지 벌어진다.

연산자별 구조는 CNN과 같다. 합성곱의 국소 불일치는 $0.06$--$0.40\%$이고 모두 최하위 비트 $\pm 1$이며, 첫
분기점도 첫 합성곱이다. 전파 값이 분류기($28$--$73\%$)보다 작은 이유에는 깊이와 과제·분포 차이가 함께 관여할 수 있으나,
본 실험은 이 요인을 분리하지 않았다. 이 초해상 조건에서도 평균 품질의 작은 차이와 코드 불일치가
함께 관측되므로, 그 분리가 분류 과제에만 한정된 현상은 아니다.
...
합성곱이 대부분이다. 격차의 원인 후보에는 융합·타일링·메모리 스케줄과 비용 항의 차이가 있으나,
이 대조만으로 특정 항의 기여를 분리할 수 없다. 표의 사이클은 두 추정기의 값이며,
실제 하드웨어에서 측정한 성능이 아니다.
...
출력 신경망은 3층뿐이며, 그 전파 값이 더 깊은 신경망의 하한이라는 보장은 없다. 비용 모델은
...
0을 포함했지만, 출력 코드의 $28.5$--$72.7\%$는 달랐다. 초해상 조건에서도 평균 PSNR의 작은
차이와 코드 불일치가 함께 관측됐다. 이는 품질의 유사성만으로 비트 일치를 보장할 수 없음을
보이며, 정확도 동등성을 입증한 결과는 아니다.
...
\textbf{비용 모델.} Vela와의 $0.29$--$0.84$배 격차를 비용 항과 스케줄 차이로 분해하고,
보정이 별도 평가 그래프에도 유지되는지 검증해야 한다. 현재 대조만으로 원인을 특정하지 않는다.
