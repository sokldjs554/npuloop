...
@torch.no_grad()
def evaluate(model: nn.Module, ds: CIFAR10NPZ, batch_size: int = 500, channels_last: bool = True, limit: int | None = None) -> float:
    model.eval()
    correct = 0; total = 0
...
def fit(model: nn.Module, ds: CIFAR10NPZ, epochs: int, lr: float = 0.1, wd: float = 5e-4, bs: int = 128, seed: int = 0,
    ...
    if resume and state_path and os.path.exists(state_path):
        ...
        log, best, start_ep = st["log"], st["best"], st["epoch"]
        print(f"{log_prefix}resumed from epoch {start_ep}", flush=True)
    t0 = time.time()
    acc = log["epochs"][-1]["test_acc"] if log["epochs"] else evaluate(model, ds, channels_last=channels_last, limit=eval_limit)
    for ep in range(start_ep, epochs):
        ...
        acc = evaluate(model, ds, channels_last=channels_last, limit=eval_limit)
        best = max(best, acc)
        rec = dict(epoch=ep + 1, train_loss=tl / tn, train_acc=tc / tn, test_acc=acc, lr=sched.get_last_lr()[0], epoch_sec=time.time() - te)
        ...
        if out:
            ...
            torch.save({"model": model.state_dict(), "opt": opt.state_dict(), "sched": sched.state_dict(), "rng": rng.bit_generator.state,
                        "torch_rng": torch.get_rng_state(), "log": log, "best": best, "epoch": ep + 1}, state_path)
    log["final_test_acc"] = acc; log["best_test_acc"] = best; log["train_minutes"] = (time.time() - t0) / 60
    if out:
