...
<!-- TABLE:E20 -->
| 스킴 | conv 층수 | 최대 K | float32 | fake-quant | 정수 엔진 | 정수 − fake (쌍 SE) | top-1 일치 | 출력 코드 불일치 | 코드가 다른 이미지 | 최대 \|Δ코드\| |
|---|---|---|---|---|---|---|---|---|---|---|
| npu-default | 53 | 4,608 | 72.41% | 72.33% | 72.38% | +0.05 ± 0.25%p | 95.01% | 57.3% | 3,925 / 3,925 | 13 |
| per-tensor | 53 | 4,608 | 72.41% | 72.13% | 72.10% | -0.03 ± 0.24%p | 95.29% | 57.9% | 3,925 / 3,925 | 11 |

ResNet-50 (keras.applications weights, ImageNet-1k), Imagenette test split at 160px, scored through the 1000-way ImageNet head. 이미지 3,925장, 클래스 1,000개 중 10개가 정답에 등장한다. 전처리는 keras 'caffe': RGB->BGR, minus per-channel mean, no scaling, 보정은 512 random Imagenette training images (seed 0). **ImageNet-scale graph and ImageNet weights; not ImageNet-1k validation accuracy** — top-1 절대값은 ImageNet 검증셋 수치와 비교할 수 없고, 이 실험이 재는 것은 모의 실행과 정수 실행의 차이다.
<!-- /TABLE:E20 -->

**같은 그림이 그대로 재현됩니다 — 규모가 20배여도.**

* **정확도는 맞습니다.** 정수 − 모의는 +0.05 ± 0.25%p(채널별)와 −0.03 ± 0.24%p(텐서별)로 둘 다 |Δ|/SE ≤ 0.2입니다.
  양자화 비용 자체(모의 − float)도 −0.08 ± 0.27%p와 −0.28 ± 0.33%p로 이 표본에서는 0과 구분되지 않습니다.
* **텐서는 안 맞습니다.** 출력 코드의 **57.3% / 57.9%**가 다르고, **3,925장 전부**에서 최소 하나가 다르며, 최대 편차는 **13레벨**입니다.
  이 저장소가 잰 값 중 가장 큰 코드 편차입니다. top-1 일치율은 95.0% / 95.3%로, 로짓이 이만큼 흔들려도 argmax는 대체로 버팁니다.
* **연산자별 구조도 그대로입니다.** conv의 국소(teacher-forced) 불일치는 0.010–0.205%(채널별) / 0.033–1.215%(텐서별),
  linear는 0.303% / 0.709%이고, **pool 2개와 add 16개는 정확히 0.000%**입니다(max pool은 재양자화가 없어 구조적으로 0).
  첫 분기점은 여기서도 **첫 합성곱(`conv1`)**입니다.
* **한 가지 단서.** 텐서별 스킴에서 conv 한 계층의 국소 불일치가 **1.215%**로, 작은 신경망들이 보였던 "1% 미만" 구간을 살짝 넘습니다.
  E15의 "conv·linear·pool은 자기 코드의 1% 미만"이라는 문장은 **그 표의 신경망들에 대한 것**이며, 이 규모에서는 그대로 옮겨지지 않습니다.
  단문판 원고 두 편에도 그 범위를 명시했습니다. 여전히 전파 값(57–58%)보다 두 자릿수 작다는 점은 같습니다.
...
덧붙여 **누산기 폭 이야기의 사각지대가 하나 줄었습니다.** E7·E18은 K ≤ 1,280에서만 쟀고 "ImageNet 규모(ResNet-50 K = 4,608)로는 옮겨지지 않는다"고 명시했습니다.
E20은 그 K = 4,608 그래프를 기준 설정(int32 누산기)에서 실제로 실행합니다. 좁은 누산기 축을 이 모델에서 다시 쓸어 보는 것은 아직 하지 않았습니다 — 한 설정당 22분이고 7개 설정이라 별도 실행이 필요합니다.
