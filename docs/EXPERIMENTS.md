...
* **정확도 차이는 어느 행에서도 0과 구분되지 않습니다.** 12행 전부 95% 신뢰구간이 0을 품고, 정답 여부가 갈리는 이미지는 10,000장 중 세 자릿수 이하입니다(ViT가 가장 많음).
  E2의 "차이" 열이 잡음이라는 것을 이제 오차 막대로 말할 수 있습니다.
...
fake-quant는 정확도 예측기로는 믿어도 되지만 텐서 검증 벡터로는 쓸 수 없다는 E2의 결론을, 이번에는 오차 막대와 test 전체 코드 수로 다시 확인한 셈입니다. "그러면 가장 큰 국소 원천을 고치면 되는가"는 E16이 답합니다.
