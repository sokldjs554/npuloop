...
> 이 문서는 모델 실험 화면 추가 **이전** 분석 Workbench의 검증 이력입니다. 현재 검증은 [MODEL_STUDY_REPORT.md](MODEL_STUDY_REPORT.md), 사용법은 [DEMO_GUIDE.md](DEMO_GUIDE.md), [STUDY_RUNNER.md](STUDY_RUNNER.md)를 따릅니다. 아래 테스트 수와 스크린샷은 해당 이전 버전의 기록이며 최신 통과 수로 사용하지 않습니다.
