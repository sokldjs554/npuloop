...
```
q_y = max( q_x[window ∩ input] )                     # 입력과 같은 s, z 를 유지
```

INT8 데이터패스에서 유일하게 **산술이 없는** 연산입니다. 출력이 입력 코드 하나를 그대로 고르는 것이므로 스케일·제로포인트가 보존되고, 재양자화도 누산도 clamp도 필요 없습니다. 그래서 이 연산의 국소 불일치는 구조적으로 0입니다 — fake-quant 쪽도 같은 격자 위의 최댓값을 고르기 때문입니다(§4의 평균 풀링이 나눗셈 반올림 때문에 1 LSB 어긋나는 것과 대조됩니다).

**패딩 처리.** 창이 입력 밖으로 나가는 칸은 값을 채우지 않고 건너뜁니다. 구현상으로는 `qmin`으로 채우는데, `qmin`은 어떤 실제 코드도 이길 수 없으므로 두 방식은 항상 같은 답을 냅니다. 이는 TFLite의 `MAX_POOL_2D`가 SAME 패딩에서 하는 것과 동일하며, `tests/test_tflite_maxpool.py`가 TFLite 참조 커널과 코드 단위로 대조합니다. torch의 `nn.MaxPool2d`가 `-inf`로 채우는 것과도 같은 결과입니다. 반면 명시적인 `ZeroPadding2D` + `valid`는 **다릅니다** — 그쪽은 제로포인트 코드가 실제로 최댓값 경쟁에 참여합니다.
...
ResNet-20 ReLU, 2,000장, 기준 = `tflite-m31-acc32-b32` 90.1% (docs/EXPERIMENTS.md의 E7 표 참고; 검증 분할로 다시 학습한 체크포인트 기준):
