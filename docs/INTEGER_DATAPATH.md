...
ResNet-20 ReLU, 2,000장, 기준 = `tflite-m31-acc32-b32` (README E7 표 참고):

| 바꾼 것 | 정확도 | 기준과 top-1 일치 | 해석 |
|---|---|---|---|
| single rounding / half-even | 91.0 / 90.9% | 99.4% | 2단계 반올림 방식은 사실상 무관 |
| truncate / floor | 89.3 / 88.7% | 95% | 반올림 없는 시프트는 체계적 편향 → −1.6~2.3%p |
| 곱셈기 15 / 7 / 3비트 | 90.9 / 90.8 / 89.7% | 99.4 / 99.0 / 95.6% | 2^k 곱셈기(3비트)면 −1.3%p |
| 바이어스 int16 / int12 | 82.9 / 9.8% | 86 / 10% | 바이어스는 `s_in·s_w` 단위라 값이 크다 — 폭을 줄이면 안 된다 |
| 누산기 int24 / int20 / int16 | 91.0 / 91.0 / 52.8% | 100 / 100 / 54% | 실제 누산값은 20비트에 들어가고, 16비트는 1,070만 회 포화 |
