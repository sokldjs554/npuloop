...
모델 변경과 학습은 PyTorch 코드와 실험 스크립트로 수행합니다. 브라우저 Workbench는 저장된 연구 결과를 조회하고 가상 조건의 비용을 다시 계산하는 화면입니다. 실제 모빌린트 칩이나 SDK에서 측정한 성능은 포함하지 않습니다.

## 모델 설계·학습에서 확인한 결과

다음 수치는 저장소의 기존 실험 결과입니다. 이번 문서 수정에서 새 학습을 수행한 것으로 표시하지 않습니다.
...
원본: [E4](../results/e4_surgery.json), [E6](../results/e6_pruning.json), [E9](../results/e9_customer_intake.json).
구현: [활성함수 변경·QAT](../experiments/e4_surgery.py), [프루닝·미세조정](../experiments/e6_pruning.py), [ViT 변경·재학습](../experiments/e9_customer_intake.py).
...
근거: [데이터 준비](../tools/prepare_cifar10.py), [Imagenette 준비](../tools/prepare_imagenette.py), [학습기](../npuloop/zoo/train.py), [실험 공통 코드](../experiments/common.py), [사용 방법](USAGE.md). 원래 학습 반복은 가중치 초기화 시드가 완전히 고정되지 않았으며, 현재 시드 처리가 과거 가중치의 동일 재생성을 보장하지는 않습니다.
...
E17에서는 초해상 입력의 float64·float32 나눗셈과 반올림 경계 차이를 입력 양자화기 결함으로 추적했습니다. 틀리는 입력과 수정 경로를 회귀 테스트로 확인했습니다. **근거: 원고 6.2절 및 [입력 양자화 테스트](../tests/test_intengine.py).**
...
| CV 및 최신 딥러닝 연구 분석 | CNN·ViT·ESPCN 실험, [선행 연구 정리](RELATED.md), E15–E17 원인 분석 | 공개 구현·선행 연구를 비교한 근거. 계속 갱신되는 최신 연구를 모두 검토했다는 주장은 하지 않음 |
| 고객사 데이터 분석·모델 학습 | ViT·Inception을 받는 가상 고객 E9, 공개 데이터셋 학습 | 실제 고객 데이터 분석·업무 수행 경력은 아님 |
| 학습 관련 툴 개발 | 공통 학습기·체크포인트·실험 저장·표 생성·양자화·검증 경로 | 구현 파일과 실행 방법으로 확인 가능 |
...
공개 Workbench와 현재 개인 연구 원고는 [GitHub Pages](https://sokldjs554.github.io/npuloop/)에서 제공합니다. 초기 로컬 통합 기록은 [INTEGRATION_REPORT.md](INTEGRATION_REPORT.md), 이후 게시 단계의 기록은 [DEPLOYMENT.md](DEPLOYMENT.md)와 `verification/publish/`를 기준으로 읽습니다. 서로 다른 단계의 테스트 수를 이번 문서 수정의 재실행 결과로 합치지 않습니다.
