...
* **그러나 텐서 단위로는 전혀 같지 않습니다.** 각 conv의 국소(teacher-forced) 불일치는 0.03~0.2%(±1 LSB; ResNet-20 기준, 여섯 모델 전체 범위는 E15의 0.02~0.44%)뿐인데, 끝까지 정수로 실행하면 깊은 레이어에서 코드의 15~30%, 최종 로짓의 50%가 달라집니다. ±1 LSB가 다음 레이어의 반올림 결정을 바꾸는 나비효과이고, 분류 결과는 그래도 거의 바뀌지 않습니다. "fake-quant와 하드웨어 결과가 다르다"는 보고를 받으면 먼저 *어느 레이어에서, 국소로, 몇 LSB* 인지 물어야 하는 이유입니다.
...
쌍 SE = 같은 이미지에서 잰 (정수 정답 − fake 정답)의 표본 표준편차 / √n. 출력 코드 불일치는 test 전체의 로짓 코드(클래스 × 이미지) 기준, 첫 분기는 250장 배치의 전파 비교에서 처음 코드가 달라지는 노드.
...
* **정확도 차이는 어느 행에서도 0과 구분되지 않습니다.** 12행 전부 |정수 − fake| ≤ 0.20%p이고 |Δ|/SE는 최대 1.6(ViT per-tensor), 95% 신뢰구간이 모두 0을 품습니다.
  정답 여부가 갈리는 이미지는 10,000장 중 15~160장뿐입니다(ViT가 가장 많음). E2의 "차이" 열이 잡음이라는 것을 이제 오차 막대로 말할 수 있습니다.
* **그러나 코드는 다릅니다.** test 전체에서 출력 로짓 코드의 **28.5%(Inception)~72.7%(ViT)**가 다르고, 출력 코드가 하나라도 다른 이미지는 95~100%입니다.
  입력이 16배 큰 Imagenette-128에서도 같은 그림입니다(코드 35.7~36.2% 불일치, top-1 일치 99.3~99.5%).
* **국소 원천은 작고, 어디서나 같은 자리에서 시작합니다.** conv의 국소(teacher-forced) 불일치는 0.02~0.44%(가장 큰 것은 MobileNetV2의 depthwise conv), 마지막 fc는 0.3~1.0%(로짓 스케일이 거칠어 ±1 LSB가 자주 갈림), avgpool은 0.7~2.7%(반올림 타이), ViT의 LayerNorm만 26%입니다.
...
<!-- TABLE:E14 -->
| 반올림 (2단계) | ResNet-20 ReLU: 기준 대비 Δacc, 시드 평균 ± 시드 표준편차 (시드별) | ResNet-20 SiLU: 기준 대비 Δacc, 시드 평균 ± 시드 표준편차 (시드별) |
|---|---|---|
| `single` | -0.05 ± 0.11%p (+0.03, -0.13; n=2) | +0.00 ± —%p (+0.00; n=1) |
| `half_even` | -0.04 ± 0.11%p (+0.03, -0.12; n=2) | +0.10 ± —%p (+0.10; n=1) |
| `truncate` | -1.00 ± 0.15%p (-0.90, -1.11; n=2) | -2.98 ± —%p (-2.98; n=1) |
| `floor` | -1.40 ± 0.04%p (-1.37, -1.42; n=2) | -3.37 ± —%p (-3.37; n=1) |
...
관찰(표는 학습 큐가 끝난 시드까지의 결과이고, 남은 시드는 큐가 끝나는 대로 추가됩니다):

* **반올림 축은 시드에 강건합니다.** ResNet-20 ReLU에서 truncate는 시드 0·1에서 −0.90 / −1.11%p, floor는 −1.37 / −1.42%p — 시드 간 차이(0.05~0.2%p)가 효과 크기보다 한 자릿수 작습니다. E7의 3비트 곱셈기 축(체크포인트에 따라 −1.3~−8.2%p)과 다른 점입니다.
* **10,000장에서는 손실이 더 크고 더 분명합니다.** E7(2,000장)의 ReLU truncate/floor −0.65 / −1.00%p는 10,000장에서 −0.90 / −1.37%p, SiLU의 −2.45 / −2.55%p는 −2.98 / −3.37%p가 되고 |Δ|/SE는 4.5~12입니다. 2,000장 추정이 우연히 낙관적이었던 셈이고, 이 축의 결론은 바뀌지 않습니다.
* **single / half-even은 여전히 잡음 안입니다.** |Δ| ≤ 0.13%p, |Δ|/SE ≤ 1.6, 기준과 이미지 단위 일치 99.0~99.4%.
* **floor가 truncate보다 항상 나쁩니다(ResNet-20).** floor의 −0.5 LSB 편향은 부호와 무관하게 한 방향으로 쌓이고, truncate(0 방향)는 음수 값에서 반대 방향이라 일부 상쇄됩니다. E7의 MobileNetV2(2,000장)에서는 순서가 반대였으므로 3시드 × 10,000장 결과로 다시 봅니다.
