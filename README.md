...
1. **모델 실험** — 구조를 바꾼 뒤 정확도가 어떻게 무너지고 회복되는지 봅니다. 기본 화면에서 E4의 변경 전 → 교체 직후 → 회복 학습 결과를 확인할 수 있습니다.
2. **NPU 분석** — 같은 모델이 가상 NPU 제약에서 어떤 연산을 호스트로 넘기고, 비용이 어디에 생기는지 확인합니다.
3. **정수 검증** — fake-quant와 실제 integer execution의 정확도뿐 아니라 내부 정수 출력이 얼마나 다른지 확인합니다.
4. **실험 기록** — 궁금한 수치를 E1–E20 원본 JSON과 검증 범위 문서까지 내려가 확인합니다.
...
- 가상 고객 시나리오의 체크포인트를 받아 진단 → 처방 → 적용까지 하는 인테이크 리포트를 냅니다
...
- **부정 결과를 지우지 않았습니다.** AdaRound는 194개 계층 **전부**에서 재구성 오차를 줄였지만 8비트 top-1은 움직이지 않았고(E19), 이 저장소의 비용 모델은 Arm Vela 대비 **5/5 낙관적**입니다(E13).
