...
|---|---|---|---|
| `single` | -0.03 ± 0.09%p (+0.03, -0.13, +0.01; n=3) | +0.01 ± 0.02%p (+0.00, +0.01, +0.03; n=3) | -0.12 ± —%p (-0.12; n=1) |
| `half_even` | -0.07 ± 0.09%p (+0.03, -0.12, -0.13; n=3) | +0.02 ± 0.09%p (+0.10, +0.04, -0.08; n=3) | +0.10 ± —%p (+0.10; n=1) |
| `truncate` | -0.98 ± 0.12%p (-0.90, -1.11, -0.92; n=3) | -2.31 ± 0.85%p (-2.98, -2.60, -1.36; n=3) | -2.34 ± —%p (-2.34; n=1) |
| `floor` | -1.42 ± 0.06%p (-1.37, -1.42, -1.48; n=3) | -2.42 ± 1.04%p (-3.37, -2.57, -1.31; n=3) | -1.86 ± —%p (-1.86; n=1) |
...
* **부호와 유의성**: truncate·floor는 측정한 14개 (모델, 시드, 모드) 조합 중 14개에서 기준보다 나쁩니다 — -3.37~-0.90%p, |Δ|/SE 4.4~11.8.
* **크기의 시드 분산**: ResNet-20 ReLU — truncate -0.98 ± 0.12%p (시드별 폭 0.21%p, n=3), floor -1.42 ± 0.06%p (시드별 폭 0.11%p, n=3) / ResNet-20 SiLU — truncate -2.31 ± 0.85%p (시드별 폭 1.62%p, n=3), floor -2.42 ± 1.04%p (시드별 폭 2.06%p, n=3).
* **floor vs truncate**: 7개 (모델, 시드) 중 floor가 더 나쁜 경우 4개, 두 모드의 차이가 각자의 쌍 SE 합보다 큰 경우 3개.
* **single / half_even**: 14개 조합 모두 |Δ| ≤ 0.13%p, |Δ|/SE ≤ 1.9, 기준과 이미지 단위 일치 99.0~99.4%.
<!-- /TABLE:E14 -->

위 요약은 `results/e14_rounding_seeds.json`에서 생성됩니다(본문의 수치가 결과와 어긋날 수 없게). 읽는 법:

* **강건한 것은 부호와 유의성이지 크기가 아닙니다.** truncate·floor는 측정한 모든 (모델, 시드)에서 기준보다 나쁘고 |Δ|/SE가 4를 넘습니다 — 여기까지는 시드를 바꿔도 흔들리지 않습니다.
  그러나 **크기는 흔들립니다**: ResNet-20 ReLU는 시드 간 폭이 0.2%p 이하(효과의 1/7)인 반면, SiLU는 truncate가 −1.36에서 −2.98%p까지 벌어져 시드 표준편차가 효과의 40%에 이릅니다.
  보고서 5장이 E7에 대해 경고한 "체크포인트를 바꾸면 크기가 달라진다"가 반올림 축에서도 그대로입니다. **"truncate는 X%p를 잃는다"는 한 시드 숫자로 인용하면 안 되고, 잃는다는 사실만 인용해야 합니다.**
...
* **floor와 truncate 중 어느 쪽이 더 나쁜지는 모델·시드마다 다릅니다.** ResNet-20 ReLU에서는 세 시드 모두 floor가 더 나쁘지만(floor의 −0.5 LSB 편향은 부호와 무관하게 한 방향으로 쌓이고 truncate는 음수에서 반대 방향이라 일부 상쇄), SiLU 시드 1·2와 MobileNetV2에서는 순서가 뒤집힙니다.
  두 모드의 차이가 각자의 쌍 SE 합을 넘는 경우도 절반이 안 되므로, **순서를 결론으로 삼을 수 없습니다.**
* **single / half_even은 기준과 구분되지 않습니다** — 모든 조합에서 |Δ|/SE < 2, 이미지 단위 일치 99%대. TFLite가 `TFLITE_SINGLE_ROUNDING` 빌드를 함께 출하하는 이유를 정량화한 셈입니다.

**시드의 한계.** `--seed N`은 데이터 순서·증강·드롭아웃을 결정하지만 **가중치 초기화는 결정하지 않습니다**(`npuloop/zoo/train.py`의 `main()`이 `build_model()`을 부른 뒤에야 `fit()` 안에서 `torch.manual_seed`를 호출합니다).
따라서 이 세 체크포인트는 서로 독립적인 실현(replicate)이 맞지만 같은 명령으로 비트 단위 재생성은 되지 않습니다. 시드 분산을 재는 목적에는 문제가 없고, 재현성 주장에는 이 한계를 붙여야 합니다.
