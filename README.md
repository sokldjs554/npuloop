...
<!-- TABLE:E7 -->
| RequantConfig | ResNet-20 ReLU: acc / 기준 대비 top-1 일치 | ResNet-20 SiLU: acc / 기준 대비 top-1 일치 |
|---|---|---|
| `tflite-m31-acc32-b32` | 91.00% / 100.0% | 90.45% / 100.0% |
| `single-m31-acc32-b32` | 91.00% / 99.4% | 90.35% / 99.1% |
| `half_even-m31-acc32-b32` | 90.85% / 99.4% | 90.45% / 99.1% |
| `truncate-m31-acc32-b32` | 89.30% / 95.2% | 88.55% / 93.4% |
| `floor-m31-acc32-b32` | 88.65% / 94.0% | 89.00% / 93.9% |
| `tflite-m15-acc32-b32` | 90.85% / 99.4% | 90.60% / 98.9% |
| `tflite-m7-acc32-b32` | 90.80% / 99.0% | 90.45% / 98.8% |
| `tflite-m3-acc32-b32` | 89.70% / 95.6% | 89.45% / 94.2% |
| `tflite-m31-acc32-b16` | 82.85% / 86.2% | 90.40% / 97.4% |
| `tflite-m31-acc32-b12` | 9.75% / 10.0% | 13.75% / 13.7% |
| `tflite-m31-acc24-b32` | 91.00% / 100.0% | 90.45% / 100.0% |
| `tflite-m31-acc20-b32` | 91.00% / 100.0% | 90.45% / 100.0% |
| `tflite-m31-acc16-b32` | 52.80% / 54.4% | 63.00% / 65.0% |

2000장 기준. 기준 구현은 `tflite-m31-acc32-b32`(gemmlowp 이중 반올림). 곱셈기 비트·누산기 폭이 줄어들 때 무엇이 먼저 무너지는지 보세요.
<!-- /TABLE:E7 -->

* **반올림 모드**: TFLite single rounding과 half-even은 기준(gemmlowp 이중 반올림)과 99.4% 일치하고 정확도 차이는 잡음 수준입니다. **truncate/floor는 일치율이 95%로 떨어지고 1.6~2.3%p를 잃습니다.** requant에서 "그냥 시프트"는 공짜가 아닙니다.
...
* **누산기 폭**: int24·int20은 포화 0회에 기준과 100% 일치 — ResNet-20의 K=576 레이어도 실제 누산값은 20비트 안에 듭니다. **int16은 1,070만 회 포화하며 52.8%로 붕괴**합니다. "최악 케이스 K·127·255 = 25비트"라는 정적 계산과 실제 분포(20비트) 사이의 여유를 이렇게 숫자로 볼 수 있습니다.
...
| silu-qat | 90.50% | 90.20% / 90.28% | 34,785 | 1,554,865 |
...
* 즉, 이 모델에서 "캘리브레이션 세트를 키우면 좋아진다"는 통념은 **측정으로는 지지되지 않습니다.** 차이는 lint가 outlier를 경고한 activation 텐서들이 아니라, 아래 MobileNetV2처럼 depthwise·범위 편차가 있는 모델에서 나타나는지 봐야 합니다.
