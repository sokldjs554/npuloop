...
<!-- TABLE:E17 -->
| 스킴 | float32 | fake-quant | 정수 엔진 | 정수 − fake (쌍 SE) | 양자화 비용 (fake − float) | 출력 픽셀 코드 불일치 | 코드가 다른 이미지 | 최대 \|Δ코드\| | conv 국소 불일치 |
|---|---|---|---|---|---|---|---|---|---|
| npu-default | 30.234 dB | 29.956 dB | 29.955 dB | -0.00064 ± 0.00004 dB | -0.279 dB | 6.3% | 3,925 / 3,925 | 3 | 0.06–0.26% |
| per-tensor | 30.234 dB | 29.567 dB | 29.568 dB | +0.00027 ± 0.00006 dB | -0.667 dB | 18.2% | 3,925 / 3,925 | 3 | 0.17–0.40% |

test 3,925장 전체, 이미지당 출력값 49,152개(3×128×128 픽셀). PSNR은 [0,1] 범위에서 이미지별로 재고 배포와 똑같이 출력을 clip한 뒤 평균한 값이다. 국소 불일치는 64장 배치의 teacher-forced 평균.
<!-- /TABLE:E17 -->

* **양자화 비용은 시뮬레이터가 맞힙니다.** fake-quant는 float32 대비 −0.279 dB(npu-default) / −0.667 dB(per-tensor)를 잃고, 정수 엔진은 그 예측에서
  0.0006 dB 안쪽으로만 벗어납니다. 양자화 비용 자체의 0.2% 수준입니다(쌍 SE 기준으로는 0과 구분되지만, dB로는 무의미한 크기입니다).
* **텐서는 여전히 못 맞힙니다.** 출력 픽셀 코드의 6.3% / 18.2%가 다르고, **3,925장 전부**에서 최소 한 픽셀이 다르며, 가장 큰 차이는 3레벨입니다.
  시뮬레이터가 내놓은 그림을 골든 이미지로 쓰면 그 비율만큼 픽셀이 틀립니다.
* **구조는 분류기와 같습니다.** conv의 국소(teacher-forced) 불일치는 0.06–0.40%, 전부 ±1 LSB이고, 첫 분기점도 첫 conv입니다. 분류기(28–73%)보다 낮은 이유는
  망이 3층이라 전파 깊이가 얕기 때문입니다 — 격차는 커널이 만드는 것이 아니라 깊이가 키운다는 E15/E16의 결론과 같은 방향입니다.
...
* **주요 분류 측정은 CIFAR-10 5종과 Imagenette 1종이며 E17은 초해상 모델입니다.** E14 반올림 축만 모델당 학습 반복 3개를 사용합니다. 정확도 차이는 각각의 대응 신뢰구간으로 해석하며, 0.2%p 이하를 일률적으로 잡음으로 판단하지 않습니다.
