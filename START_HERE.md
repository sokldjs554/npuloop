...
이 ZIP은 수정된 전체 프로젝트입니다. 별도 패치를 적용하지 않습니다.
새 폴더에 압축을 풀고 `npuloop/docs/index.html`을 브라우저로 엽니다.
Python·Node·웹 서버·API 키는 화면 실행에 필요하지 않습니다.
...
실험 기록에서 전체 E1–E17을 조회하고 각 기록의 JSON을 열거나 저장할 수 있습니다.
...
연구 결과 JSON, 정수 엔진, 번들 체크포인트, 논문 PDF는 이전 통합본의 값을 유지했습니다.
...
자세한 실행 결과와 범위는 `docs/DEMO_WORKBENCH_REPORT.md`에 기록합니다.
원격 GitHub나 공개 데모는 변경하지 않았습니다.
